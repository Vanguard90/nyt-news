...
describe('Header', () => {
    ...
    describe('displays correct date', () => {

        it('for January & Monday', () => {
            MockDate.set('1/1/2018');
            expect(shallow(<Header />).find('p').html()).toContain('Monday, 1 January, 2018');
            // the reason why I'm setting a new shallow(<Header />) at every test is to instantiate it every time seperately.
        });

        it('for February & Wednesday', () => {
            MockDate.set('2/7/2018');
            expect(shallow(<Header />).find('p').html()).toContain('Wednesday, 7 February, 2018');
        });
    
        it('for March & Thursday', () => {
